  ...
  async _switchToNextAuth() {
    ...
    try {
      ...
      try {
        ...
        this.logger.info(
          `✅ [Auth] 成功回退到账号 #${previousAuthIndex}，服务已恢复！`
        );
        this.failureCount = 0;
        this.usageCount = 0;
        this.logger.info("[Auth] 所有计数已重置。");
        ...
        return {
          ...
          failedIndex: nextAuthIndex,
        }; // 返回回退成功状态
      } catch (fallbackError) {
        this.logger.error(
          `FATAL: ❌❌❌ [Auth] 紧急回退到账号 #${previousAuthIndex} 失败！`
        );
        this.logger.error(
          `FATAL: [Auth] 回退失败原因: ${fallbackError.message}`
        );
        this.isAuthSwitching = false;
        fallbackError.message = `切换到 #${nextAuthIndex} 失败后，尝试回退到 #${previousAuthIndex} 也失败了。回退失败原因: ${fallbackError.message}`;
  ...
  async _handleRequestFailureAndSwitch(errorDetails, res) {
    ...
    if (isImmediateSwitch || isThresholdReached) {
      if (isImmediateSwitch) {
        this.logger.warn(
          `🔴 [Auth] 收到报错码 ${errorDetails.status}，触发立刻切换...`
        );
      } else {
        this.logger.warn(
          `🔴 [Auth] 达到失败阈值 (${this.failureCount}/${this.config.failureThreshold})，触发自动切换...`
        );
      }
      ...
      try {
        ...
        // 根据返回结果，组织不同的提示信息
        if (result.success) {
          // 这个分支对应“普通切换成功”，例如从好账号1切换到好账号2
          userMessage = `✅ 自动切换成功：服务已切换至可用账号 #${result.newIndex}，请重试。`;
        } else if (result.fallback) {
          // 这个分支对应“回退成功”，并明确指出哪个账号出了问题
          userMessage = `⚠️ 切换到账号 #${result.failedIndex} 失败，已自动回退到原账号 #${result.newIndex}。请检查目标账号。`;
        }

        this.logger.info(`[Auth] ${userMessage}`);
        if (res && userMessage) this._sendErrorChunkToClient(res, userMessage);
      } catch (error) {
        let userMessage = `❌ 致命错误：发生未知切换错误: ${error.message}`;

        if (error.message.includes("Only one account is available")) {
          userMessage = "❌ 切换失败：有一个可用账号，无法切换。";
...
class ProxyServerSystem extends EventEmitter {
  ...
  _loadConfiguration() {
    ...
    this.logger.info(
      `  失败切换: ${
        this.config.failureThreshold > 0
          ? `连续失败 ${this.config.failureThreshold} 次后切换`
          : "已禁用"
      }`
    );
    this.logger.info(
  ...
  _createExpressApp() {
    ...
    app.get("/", (req, res) => {
      ...
      if (isHealthCheck) {
        this.logger.info(
          `[Auth] 收到Hugging Face健康检查 (User-Agent: ${userAgent})，已忽略。`
        );
      ...
      const statusHtml = `
        ...
        <head>
          ...
          <meta http-equiv="refresh" content="5">  <title>代理服务状态 (自动刷新中...)</title>
          <style>
            body { font-family: 'SF Mono', 'Consolas', 'Menlo', monospace; background-color: #f0f2f5; color: #333; padding: 2em; }
            .container { max-width: 800px; margin: 0 auto; background: #fff; padding: 2em; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
...
--- 账号状态 ---
<span class="label">扫描到的账号</span>: [${initialIndices.join(
        ", "
      )}] (总数: ${initialIndices.length})
<span class="label">格式正确 </span>: [${availableIndices.join(", ")}] (总数: ${
        availableIndices.length
      })
<span class="label">格式错误 </span>: [${invalidIndices.join(", ")}] (总数: ${
        invalidIndices.length
      })

<span class="label">当前账号</span>: #${requestHandler.currentAuthIndex}
<span class="label">轮换次数</span>: ${requestHandler.usageCount} / ${
        config.switchOnUses > 0 ? config.switchOnUses : "N/A"
      }
<span class="label">失败计数</span>: ${requestHandler.failureCount} / ${
